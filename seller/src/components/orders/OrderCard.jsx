    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="border-t pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          {order.status !== 'pending' && ( // hide pending button if already set
            <button
              onClick={() => updateOrderStatus(order.id, 'pending')}
              className="px-4 py-2 bg-orange-100 text-orange-800 rounded-lg hover:bg-orange-200 transition text-sm"
            >
              Pending
            </button>
          )}
          ...
          <button
            onClick={() => {
              const reason = prompt('Enter reason for flagging this buyer:');
              if (reason?.trim()) flagBuyer(order.buyerEmail, reason);
            }}
            className="ml-auto px-4 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 transition text-sm flex items-center gap-2"
          >
            <Flag className="w-4 h-4" />
